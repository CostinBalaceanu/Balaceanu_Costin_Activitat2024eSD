...
void main() {
	...
	*/
	...
	const char* numeFisier = "biblioteci.txt"; 
	int nrBiblioteciFisier;
	Biblioteca* biblioteci = citesteBiblioteciDinFisier(numeFisier, &nrBiblioteciFisier);
	if (biblioteci != NULL) {
		printf("\nBiblioteci citite din fisier:\n");
		afisareVectorBiblioteci(biblioteci, nrBiblioteci);
		dezalocare(&biblioteci, &nrBiblioteci);
	}

	Biblioteca bibliotecaSalvata = initializareBiblioteca(33, "Biblioteca Salvata", "Bucuresti", 103);
	salveazaBibliotecaInFisier(bibliotecaSalvata, "biblioteca_salvata.txt");
	free(bibliotecaSalvata.denumire);
	free(bibliotecaSalvata.locatie);

	salveazaVectorBiblioteciInFisier(vectorBiblioteci, nrBiblioteci, "vector_biblioteci.txt");
